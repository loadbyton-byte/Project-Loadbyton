...
import { IconCheck, IconLayers, IconFile, IconPackage, IconWallet, IconTruck, IconWarehouse, IconLock, IconShield, IconAlert, IconGavel, IconClose, IconCompass, IconSync, IconMail } from './icons.jsx';

export const TERMS_LAST_UPDATED = '2026-08-16';

// The full Terms of Service body — shared between the standalone /terms
// marketing page (Terms.jsx, wrapped in its own hero) and TermsModal.jsx
// (read-inline popup, so a shipper/carrier never has to leave the form
// they're mid-filling just to see what they're agreeing to). One copy of
// the legal text, two presentations.
export default function TermsContent({ compact = false }) {
  const sectionSpacing = compact ? 'space-y-8' : 'space-y-10';
  return (
    <>
      <div className={compact ? '' : 'container-page max-w-3xl'}>
      ...
      <div className={compact ? `mt-8 ${sectionSpacing}` : `container-page max-w-3xl mt-16 md:mt-20 ${sectionSpacing}`}>
        <Reveal>
        ...
        <Reveal delay={480}>
          ...
          <p className="mt-3 text-sm leading-relaxed text-ink-secondary">
            ...
            of 2021 (PDPL) and our <a href="/privacy" target="_blank" rel="noreferrer" className="text-brand-secondary hover:underline">Privacy Policy</a>.
