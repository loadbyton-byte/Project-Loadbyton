            ...
            matches.map((m) => (
              <Link key={m.id} to={`/jobs/${m.id}`} className="block rounded-md border px-3 py-2.5 hover:bg-raised" style={{ borderColor: 'var(--border-default)' }}>
                <div className="flex items-center justify-between gap-2">
                  ...
                  <Badge color={m.matchType === 'coords' ? 'success' : 'neutral'} dot={false}>{m.matchType === 'coords' ? `${m.distanceKm} km away` : 'Same emirate'}</Badge>
                </div>
                ...
                <p className="mt-0.5 flex items-center gap-1.5 text-xs text-ink-secondary">
                  {m.shipper_company} <Badge color="accent" dot={false} className="text-[10px]">★ {m.shipper_rating?.toFixed(1) || '—'}</Badge> · {formatAED(m.max_budget_aed)} target
                </p>
              </Link>
            ))
          )}
