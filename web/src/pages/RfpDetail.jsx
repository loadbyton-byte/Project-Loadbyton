...
export default function RfpDetail() {
  ...
  async function fetchData() {
    ...
    try {
      const [rfpData, bidsData] = await Promise.all([
        api.getRfp(id),
        api.get(`/rfps/${id}/bids`).catch(() => ({ bids: [] })),
      ]);
      setRfp(rfpData);
  ...
  const isOwner = rfp.shipper_id === api.me ? true : false; // Will be replaced by actual user check
              ...
              {bids.map((bid) => (
                <div key={bid.id} className="flex flex-wrap items-center justify-between gap-3 rounded-md border px-4 py-3" style={{ borderColor: bid.status === 'ACCEPTED' ? 'var(--status-success)' : 'var(--border-default)' }}>
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-ink-muted">{bid.carrier_company || 'Unknown carrier'} · {bid.eta_hours ? `${bid.eta_hours}h ETA` : 'No ETA'}</p>
                  </div>
                    ...
                    {isOwner && rfp.status === 'OPEN' && bid.status === 'PENDING' && (
                      <Button variant="accent" size="sm" onClick={async () => {
                        try {
                          ...
                          addToast('RFP awarded', 'success');
                          fetchData();
                        } catch (e) { addToast(e.message || 'Failed to award', 'error'); }
                      }}>Award</Button>
                    )}
                  </div>
                </div>
