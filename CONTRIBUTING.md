...
## Stack
- Node 22+ (`node:sqlite` built-in — do not add a database driver)
- Express + bcryptjs only. Keep dependencies minimal.
- React 18 + Vite + Tailwind 3 for the SPA.
...
## Testing
- `cd server && npm test` — isolated temp DB per run, real HTTP, no mocks.
- `cd web && npm run build` must succeed before any frontend change.
