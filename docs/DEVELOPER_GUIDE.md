...
Both are opt-in via build-time frontend env vars — unset, they no-op silently
(no broken tags, no console errors). Set as **Vercel project env vars**
(Project → Settings → Environment Variables), not in the server's `.env`,
since Vercel builds `web/` independently of the API.

| Var | Purpose |
|---|---|
| `VITE_GA_MEASUREMENT_ID` | Google Analytics 4 property ID (`G-XXXXXXXXXX`). Until set, `gtag.js` is never requested (see `web/index.html`'s guard). |
| `VITE_GSC_VERIFICATION` | Search Console HTML-tag verification content string. |
| `SITE_ORIGIN` | Overrides the canonical/sitemap origin (defaults to `https://loadbyton.com`) — only needed for a staging domain. |

**One-time setup (needs your own Google login — this part can't be scripted for you):**

1. **GA4**: [analytics.google.com](https://analytics.google.com) → Admin → Create Property → name it "Loadbyton" → Web data stream → URL `https://loadbyton.com` → copy the **Measurement ID** (`G-…`) it gives you → set as `VITE_GA_MEASUREMENT_ID` in Vercel → redeploy.
2. **Search Console**: [search.google.com/search-console](https://search.google.com/search-console) → Add property → "URL prefix" → `https://loadbyton.com` → choose the **HTML tag** verification method → copy the `content="…"` value → set as `VITE_GSC_VERIFICATION` in Vercel → redeploy → click Verify in Search Console.
