...
router.post('/api/jobs/:id/location', auth(['CARRIER']), async (req,res)=>{
  ...
  await db.prepare(`INSERT INTO location_logs (job_id, carrier_id, lat,lng,speed,heading) VALUES (?,?,?,?,?,?)`).run(job.id, req.user.id, Number(lat), Number(lng), speed!=null?Number(speed):null, heading!=null?Number(heading):null);
  res.json({ ok:true });
});
router.get('/api/jobs/:id/locations', auth(), async (req,res)=>{
  const job = await db.prepare('SELECT * FROM jobs WHERE id=?').get(req.params.id);
  if(!job) return sendError(res,404,'Job not found');
  ...
  const rows = await db.prepare(`SELECT * FROM location_logs WHERE job_id=? ORDER BY recorded_at DESC LIMIT 100`).all(job.id);
