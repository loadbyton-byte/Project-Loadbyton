...
router.post('/api/webhooks/stripe/mock-confirm', async (req,res)=>{
...
router.post('/api/jobs/:id/release-payout', auth(['SHIPPER','ADMIN']), async (req,res)=>{
  ...
  await db.prepare(`UPDATE payouts SET status='RELEASED', released_at=datetime('now'), processor_payout_status='SENT', processor_payout_ref=?, transfer_reference=? WHERE id=?`).run(tr.id, tr.id, payout.id);
