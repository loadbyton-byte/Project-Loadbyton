...
router.post('/api/jobs/:id/compliance', auth(['SHIPPER','ADMIN']), (req,res)=>{
  ...
  const r=db.prepare(`INSERT INTO compliance_declarations (job_id, hs_code, manifest_hash, zk_proof, status) VALUES (?,?,?,?, 'PENDING')`).run(job.id, String(hs), manifestHash, zkProof);
  ...
  const decl=db.prepare('SELECT * FROM compliance_declarations WHERE id=?').get(Number(r.lastInsertRowid));
  res.status(201).json({ declaration: decl, manifest: m });
});
router.post('/api/compliance/:id/clear', auth(['ADMIN']), (req,res)=>{
  db.prepare(`UPDATE compliance_declarations SET status='CLEARED', cleared_at=datetime('now') WHERE id=?`).run(req.params.id);
  const d=db.prepare('SELECT * FROM compliance_declarations WHERE id=?').get(req.params.id);
  if(!d) return sendError(res,404,'Declaration not found');
  res.json({ declaration: d });
});
router.get('/api/jobs/:id/compliance', auth(), (req,res)=>{
  const rows=db.prepare('SELECT * FROM compliance_declarations WHERE job_id=? ORDER BY created_at DESC').all(req.params.id);
