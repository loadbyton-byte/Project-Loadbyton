...
router.get('/api/fleet/overview', auth(['CARRIER']), (req,res)=>{
  // thin proxy to /api/carrier/fleet for fleet ops UI
  const jobs=db.prepare('SELECT * FROM jobs WHERE carrier_id=? ORDER BY created_at DESC LIMIT 50').all(req.user.id);
