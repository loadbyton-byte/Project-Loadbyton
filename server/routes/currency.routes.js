...
router.post('/api/jobs/:id/currency', auth(['SHIPPER']), (req,res)=>{
  ...
  db.prepare(`UPDATE jobs SET country_code=?, currency=?, tax_rate_bps=?, tax_amount=?, updated_at=datetime('now') WHERE id=?`).run(cc, cur, tax.vatBps, tax.vat, job.id);
