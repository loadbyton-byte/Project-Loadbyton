...
test('issueInvoice retries and still succeeds when its computed number collides with an existing invoice', async () => {
  ...
  const funded = await admin0.post('/api/admin/confirm-receipt', { jobId });
  assert.equal(funded.status, 200, funded.raw);
  const drv = await carrier.patch(`/api/jobs/${jobId}/driver`, { driverName: 'Test Driver', driverPhone: '0559990000' });
  assert.equal(drv.status, 200, drv.raw);
  const pu = await carrier.patch(`/api/jobs/${jobId}/status`, { status: 'PICKED_UP' });
  assert.equal(pu.status, 200, pu.raw);
  const it = await carrier.patch(`/api/jobs/${jobId}/status`, { status: 'IN_TRANSIT' });
  assert.equal(it.status, 200, it.raw);
