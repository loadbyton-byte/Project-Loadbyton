...
function writeAuditChained(req, {userId, action, details, entityType, entityId, beforeState, afterState}){
  const prev = db.prepare('SELECT hash FROM audit_log ORDER BY id DESC LIMIT 1').get()?.hash || 'GENESIS';
